...
def test_platform_supervisor_snapshot_covers_ai_os_domains():
    ...
    assert "configured_key_count" in domains["providers"]["metrics"]
    assert "secret-value-that-must-not-leak" not in json.dumps(domains["providers"])
    ...
    assert domains["computer_control"]["metrics"]["group_status"]["desktop_agent_loop"] == "ready"
