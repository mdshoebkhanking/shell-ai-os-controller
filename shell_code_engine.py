...
def _strip_build_brief_noise(app_type: str, project_name: str) -> str:
    text = str(app_type or "").strip()
    text = re.sub(r"\bdo not echo the request text as page copy\.?", " ", text, flags=re.I)
    text = re.sub(r"\bbuild\s+(?:a\s+)?(?:polished\s+responsive\s+)?(?:full-stack\s+)?(?:website|app)\s+(?:for|about)?\s*", " ", text, flags=re.I)
    text = re.sub(r"\binclude\b.*$", " ", text, flags=re.I | re.S)
    text = re.sub(
    ...
    return text or project_name.replace("_", " ")


def _fallback_html_body(project_name: str, app_type: str) -> str:
    title = project_name.replace("_", " ").title()
    subject = _strip_build_brief_noise(app_type, project_name).title()
    lower = str(app_type or "").lower()
    is_app = "full-stack app" in lower or re.search(r"\b(app|dashboard|tracker|manager|todo|crm)\b", lower)
    if is_app:
        return f"""
<nav class='site-nav'><div class='brand'>{title}</div><div class='nav-links'><a href='#dashboard'>Dashboard</a><a href='#workflow'>Workflow</a><a href='#status'>Status</a></div></nav>
<header id='dashboard' class='hero'><div><p class='eyebrow'>Shell Generated App</p><h1>{subject} Command Center</h1><p>A focused workspace for managing {subject.lower()} with clear actions, live status, and responsive controls.</p><button class='btn'>Open Dashboard</button></div><div class='hero-badge'><strong>Live Workspace</strong><small>Tasks, records, and activity stay organized in one place.</small></div></header>
<main>
  <section id='workflow'><h2>Core Workflow</h2><div class='card-grid'><article class='card'><h3>Create</h3><p>Add new records quickly with concise fields and visible validation.</p></article><article class='card'><h3>Track</h3><p>Review active, pending, and completed work without switching context.</p></article><article class='card'><h3>Act</h3><p>Use direct controls for update, complete, archive, and export actions.</p></article></div></section>
  <section id='status'><h2>Operational View</h2><p>The layout is built for repeated daily use: readable metrics, stable navigation, and compact responsive panels.</p><div class='feature-list'><span>Persistent backend</span><span>Responsive UI</span><span>Empty states</span><span>Error states</span></div></section>
</main>
<footer id='contact' class='site-footer'><small>{title} generated by Shell AI</small></footer>
"""
    return f"""
<nav class='site-nav'><div class='brand'>{title}</div><div class='nav-links'><a href='#home'>Home</a><a href='#services'>Services</a><a href='#contact'>Contact</a></div></nav>
<header id='home' class='hero'><div><p class='eyebrow'>Shell Generated Website</p><h1>{subject}</h1><p>A polished responsive landing page with a clear offer, useful sections, and a direct call to action.</p><button class='btn'>Get Started</button></div><div class='hero-badge'><strong>Premium Web Presence</strong><small>Designed for quick scanning and confident action.</small></div></header>
<main>
  <section id='services'><h2>What This Page Highlights</h2><div class='card-grid'><article class='card'><h3>Clear Value</h3><p>The opening section explains what makes {subject.lower()} worth attention.</p></article><article class='card'><h3>Useful Details</h3><p>Services, benefits, and proof points are grouped into readable blocks.</p></article><article class='card'><h3>Action Path</h3><p>The CTA gives visitors a simple next step instead of leaving them guessing.</p></article></div></section>
  <section id='contact'><h2>Contact Ready</h2><p>Add phone, email, location, pricing, menu, booking, or portfolio details here depending on the business.</p><div class='feature-list'><span>Responsive</span><span>Fast</span><span>Accessible</span><span>Easy to edit</span></div></section>
</main>
<footer class='site-footer'><small>{title} generated by Shell AI</small></footer>
"""


def _offline_website_blueprint(project_name: str, app_type: str) -> Dict[str, Any]:
    try:
        from shell_make_modes import offline_basic_website_blueprint

        return dict(offline_basic_website_blueprint(app_type or project_name))
    except Exception as exc:
        logger.warning("Offline website blueprint failed: %s", exc)
        return {}
...
async def create_fullstack_app_tool(project_name: str, app_type: str = "modern_webapp") -> str:
    ...
    try:
        ...
        is_website_request = bool(re.search(r"\b(website|webpage|web\s+page|landing\s+page|site|portfolio)\b", str(app_type), re.I))
        if not blueprint and is_website_request:
            offline_blueprint = _offline_website_blueprint(project_name, app_type)
            if offline_blueprint:
                blueprint = {
                    "frontend": {
                        "html_body": offline_blueprint.get("html_body"),
                        "css_vars": offline_blueprint.get("css_vars"),
                        "js_logic": offline_blueprint.get("js_logic"),
                    },
                    "meta": offline_blueprint.get("meta", {}),
                }
        ...
        html_body = str(
            ...
            or _fallback_html_body(project_name, app_type)
        )
