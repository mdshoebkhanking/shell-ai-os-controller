...
# =====================================================================
#  TTS Speaker — Neural voice via edge-tts (Microsoft Neural TTS)
# =====================================================================
...
# Neural voice presets — natural Aether-style voices
_NEURAL_VOICES = {
    ...
    # Settings exposes Gemini-style persona names. Map them to stable
    # edge-tts voices so selecting a persona does not break local TTS.
...
class TTSSpeaker(QThread):
    """Background thread — speaks Shell replies using the selected voice mode."""
    ...
    def __init__(self, parent=None):
        ...
        self._enabled = True
        self._rate = "+8%"      # edge-tts rate string
        self._volume = "+0%"
        self._running = True
    ...
    def set_rate(self, rate_pct):
        """Set speech rate from a UI percent or edge-tts relative string."""
        self._rate = self._edge_relative_percent(
            rate_pct, neutral=100, min_delta=-90, max_delta=100)

    def set_volume(self, vol_pct):
        """Set volume from a UI percent or edge-tts relative string."""
    ...
    def speak(self, text, force=False):
        ...
        # Strip markdown, code blocks, emojis for cleaner speech
    ...
    def _do_speak(self, text):
        ...
        Cloud mode must use Gemini TTS. It should not silently fall back to a
        robotic local voice because that makes the UI claim one voice while the
        user hears another.
        """
        self._last_error = ""
        try:
        ...
        if _EDGE_TTS_AVAILABLE and engine in {"edge", "edge-tts", "auto"}:
            try:
                tmp_path = os.path.join(self._temp_dir, f"shell_voice_{id(self)}_{_time.monotonic_ns()}.mp3")

                # Generate neural speech to MP3
                loop = asyncio.new_event_loop()
                try:
                    comm = _edge_tts.Communicate(text, self._voice,
                ...
                logging.warning(f"edge-tts failed: {e}")
    ...
    def _run_speech_command(self, argv, timeout=45):
        ...
        try:
            ...
            self._emit_latency("playback_started", started, command=os.path.basename(str(argv[0])))
            _stdout, stderr = proc.communicate(timeout=timeout)
            if proc.returncode != 0:
                err = (stderr or b"").decode("utf-8", errors="replace").strip()
                self._last_error = err or f"{os.path.basename(str(argv[0]))} exited with code {proc.returncode}"
            ...
            logging.warning(f"TTS command failed: {e}")
            return False
        finally:
            self._current_process = None
    ...
    def _speak_system(self, text):
        ...
        if system == "windows":
            # Persistent SAPI via pyttsx3 is much faster than spawning a new
            # PowerShell/System.Speech process for every reply. PowerShell is
            # retained as a last-ditch fallback when pyttsx3 is unavailable.
            ...
            if powershell:
                ...
                return self._run_speech_command([powershell, "-NoProfile", "-NonInteractive", "-Command", cmd])
    ...
    def _play_audio_file(self, path):
        ...
        for player in ("ffplay", "mpg123", "mpv"):
            if shutil.which(player):
                if player == "ffplay":
                    return self._run_speech_command([player, "-nodisp", "-autoexit", "-loglevel", "quiet", path])
...
# =====================================================================
#  VoiceListener — Real mic input using sounddevice + Google Speech API
# =====================================================================
...
class VoiceListenerThread(QThread):
    """Listens to microphone, detects speech, converts to text via Google STT.
    Uses sounddevice for audio capture (no PyAudio needed)."""

    text_recognized = pyqtSignal(str)      # Emits recognized speech text
    amplitude_changed = pyqtSignal(float)  # Emits 0.0-1.0 for waveform
    listening_started = pyqtSignal()
    listening_stopped = pyqtSignal()
    status_changed = pyqtSignal(str)       # "LISTENING", "PROCESSING", "ERROR"
    ...
    def __init__(self, parent=None):
        ...
        self._channels = 1
        self._silence_threshold = 0.02     # Amplitude below this = silence
        self._speech_timeout = 2.0         # Seconds of silence to end utterance
        self._min_speech_duration = 0.5    # Minimum speech length in seconds
        self._max_speech_duration = 30.0   # Maximum recording length
    ...
    def run(self):
        ...
        while self._running:
            ...
            try:
                ...
                # Wait for speech to start (voice activity detection)
                speech_frames = []
                silence_count = 0
                ...
                while self._running and not self._muted:
                    try:
                        audio_chunk = _sd.rec(
                            chunk_size, samplerate=self._sample_rate,
                ...
                wav_buffer.seek(0)
                with _sr.AudioFile(wav_buffer) as source:
                ...
                try:
                    ...
                    pass  # Couldn't understand — just continue listening
                except _sr.RequestError as e:
                    self.error_occurred.emit(f"Speech API error: {e}")

            except Exception as e:
                logging.warning(f"VoiceListener error: {e}")
                self.msleep(500)
