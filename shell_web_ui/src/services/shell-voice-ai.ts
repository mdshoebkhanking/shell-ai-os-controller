...
import { getHistory, retrieveCoreMemory, saveCoreMemory, saveMessage } from './shell-ai-brain'
...
You are **Shell AI**, a high-performance AI agent. You don't just talk; you **execute**.
...
- **Example:** "Shell, find my code and send it on WhatsApp."
...
`
    ...
    const finalSystemInstruction = SHELL_SYSTEM_INSTRUCTION + contextPrompt
    ...
    this.openPromise = new Promise((resolve, reject) => {
      if (!this.socket) {
        reject(new Error('Gemini socket was not created.'))
        return
      }
      const timeout = window.setTimeout(() => reject(new Error('Gemini Live connection timed out.')), 8000)
          ...
          tools: [
            {
              functionDeclarations: [
                  ...
                  parameters: {
                    ...
                    properties: {
                      title: {
                        type: 'STRING',
                        description:
                          'A short, descriptive title for the note (e.g., "Project_Shell_Plan").'
                      },
                      content: {
                        type: 'STRING',
                        description:
                  ...
                  parameters: {
                    ...
                    properties: {
                      source_path: {
                        type: 'STRING',
                        description:
                          'The absolute file path on the PC (e.g., "C:/Users/ShellUser/Desktop/document.pdf").'
                      },
                      dest_path: {
                        type: 'STRING',
                        description:
                  ...
                  parameters: {
                    ...
                    properties: {
                      ...
                      custom_text: {
                        type: 'STRING',
                        description:
                          'If rewriting text, generate a highly cinematic, hacker-style headline to inject into the website. (e.g., "SHELL AI HAS TAKEN OVER", or whatever the user requested).'
                      }
                    },
                    required: ['url', 'mode']
                  }
                ...
                },
                {
                  ...
                  description:
                    'ACTION: Spawns the Shell Live Forge and generates a full, highly animated, real-time website using Tailwind CSS and GSAP. Use this when the user asks you to build a landing page, a portfolio, a 3D site, or a complex web interface.',
                  parameters: {
                    type: 'OBJECT',
                    properties: {
                      prompt: {
                  ...
                  parameters: {
                    ...
                    properties: {
                      base_directory: {
                        type: 'STRING',
                        description:
                          'The absolute path of the root folder being sorted (e.g., "C:\\Users\\ShellUser\\Downloads").'
                      },
                      files_to_sort: {
                        type: 'ARRAY',
                        items: {
                ...
                },
                {
                  ...
                  description:
                    'Instantly locks the Shell OS system, disconnects the AI, and returns the user to the secure biometric lock screen. Use this strictly when the user says "Lock the system", "Lock down", or "Activate Sentry Mode".',
                  parameters: {
                    type: 'OBJECT',
                    properties: {}
                  }
          ...
          generationConfig: {
            ...
            speechConfig: {
              voiceConfig: {
                prebuiltVoiceConfig: {
                  voiceName:
                    localStorage.getItem('shell_voice_profile') === 'FEMALE' ? 'Aoede' : 'Puck'
                }
              }
            }
          },
        ...
        if (serverContent) {
          ...
          if (serverContent.turnComplete || serverContent.interrupted) {
            ...
            if (this.aiResponseBuffer.trim()) {
              await saveMessage('model', this.aiResponseBuffer.trim())
              this.aiResponseBuffer = ''
            }
          }
        }
  ...
  async forceSpeak(text: string): Promise<void> {
...
export const shellService = new GeminiLiveService()
