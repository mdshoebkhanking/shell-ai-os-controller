...
const PhoneView = ({ glassPanel }: { glassPanel?: string }) => {
  const [ip, setIp] = useState(() => localStorage.getItem('shell_adb_ip') || '')
  const [port, setPort] = useState(() => localStorage.getItem('shell_adb_port') || '5555')
  ...
  useEffect(() => {
    window.electron.ipcRenderer.invoke('adb-get-history').then((data) => {
      ...
      if (history.length > 0 && !hasAutoConnected.current) {
        hasAutoConnected.current = true

        const lastDevice = history[history.length - 1]
  ...
  const handleManualConnect = () => {
    localStorage.setItem('shell_adb_ip', ip)
    localStorage.setItem('shell_adb_port', port)
          ...
          <div className="flex flex-wrap justify-center gap-10">
            {deviceHistory.map((dev, i) => (
              <button
                ...
                className="w-52 h-72 bg-black border-8 border-zinc-900 rounded-[2.5rem] relative flex flex-col p-2 group hover:border-emerald-500/50 transition-all duration-500 shadow-2xl hover:shadow-[0_0_40px_rgba(16,185,129,0.2)]"
              >
            ...
            <button
              ...
              className="w-52 h-72 bg-transparent border-4 border-dashed border-zinc-800 hover:border-emerald-500/50 rounded-[2.5rem] flex flex-col items-center justify-center group transition-all duration-500 hover:bg-emerald-500/5"
            >
        ...
        <div className="bg-[#111] border border-white/5 rounded-2xl p-6 flex flex-col h-full shadow-lg">
          ...
          <div className="mb-6 p-4 bg-purple-500/5 border border-purple-500/20 rounded-xl">
            <p className="text-[10px] text-purple-400 font-mono leading-relaxed text-center">
              Shell AI is listening via the primary neural audio interface. Voice commands for app
